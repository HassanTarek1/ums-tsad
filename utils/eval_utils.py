...
def _valid_prediction_inputs(ranking_obj, 
    ...
    if (len(ranking_obj.predictions.keys()) != n_models) or\
         ...
         VALID_MODELS = False
         if verbose:
            print(f'Fewer than {n_models} algorithm trained! Skipping...')
    ...
    for mn in model_names:
        if (len(ranking_obj.predictions[mn].keys()) != n_quantities) and\
            ...
            if verbose:
                print(f'Fewer than {n_quantities} quantities or {n_synthetic_quantities} synthetic quantities algorithm trained! Skipping...')
...
def pool_predictions_of_entities(entities,
    ...
    ignore_timesteps: int
        Ignore the first few timesteps to allow algorithm to warm up
    """
    ...
    for entity in tqdm(entities):
        ...
        # NOTE: We are not normalizing the anomaly scores of the algorithm
...
def evaluate_models_pooled(entities: List[str],
    ...
    """
    Evaluates algorithm in terms of
    """
